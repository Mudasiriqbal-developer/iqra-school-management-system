...
const RecordBookPaymentModal = ({ isOpen, record, onSuccess, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="relative bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-gray-100 overflow-hidden transform transition-all animate-in fade-in zoom-in duration-200">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 text-left">
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5 space-y-2">
            <div className="flex justify-between items-start">
              <div>
                ...
                <span className="text-[11px] text-slate-500">
                  Reg No: <span className="font-semibold text-slate-700">{student?.registrationNumber || 'N/A'}</span>
                </span>
              </div>
              <div className="text-right">
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Class / Sec</span>
            ...
            <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200 text-center">
              <div className="bg-white rounded-lg p-2 border border-slate-150">
                ...
                <span className="text-xs font-black text-slate-700">Rs. {totalAmount.toLocaleString()}</span>
              </div>
              <div className="bg-white rounded-lg p-2 border border-slate-150">
                <span className="text-[10px] text-slate-400 uppercase font-bold block">Already Paid</span>
                <span className="text-xs font-black text-emerald-600">Rs. {amountPaid.toLocaleString()}</span>
              </div>
              <div className="bg-amber-50 rounded-lg p-2 border border-amber-200/70">
                <span className="text-[10px] text-amber-700 uppercase font-black block">Balance Due</span>
                <span className="text-xs font-black text-amber-900">Rs. {remainingBalance.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-sm font-black text-sky-700 mt-0.5">Rs. {remainingBalance.toLocaleString()}</span>
              </button>
              ...
              >
                ...
                <span className="text-xs text-slate-400 mt-0.5">Enter amount below</span>
              </button>
            </div>
          </div>
            ...
            <div className="space-y-1.5 animate-in fade-in duration-150">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  className="w-full pl-12 pr-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500 text-sm font-bold text-slate-800 bg-white"
                />
              </div>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500 text-sm font-semibold text-slate-800 bg-white"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 block">
              Remarks / Transaction Reference <span className="text-slate-400 font-normal">(Optional)</span>
            </label>
            <input
              type="text"
              value={note}
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500 text-xs font-medium text-slate-800 bg-white"
            />
          </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-3 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 rounded-xl bg-navy-900 hover:bg-navy-800 text-white font-bold text-xs flex items-center space-x-2 shadow-sm shadow-navy-950/20 disabled:opacity-50 transition-all"
            >
              {loading ? (
                <>
