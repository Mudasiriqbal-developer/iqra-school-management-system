...
import { LayoutDashboard, Calendar, Award, Settings, Key, Eye, EyeOff, ShieldAlert, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.error('Error fetching teacher profile for settings:', error);
      } finally {
    ...
    return (
      ...
      >
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#00215E]"></div>
        </div>
      </DashboardLayout>
    );
  }
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-6 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex flex-col items-center text-center space-y-3">
                <div className="h-20 w-20 rounded-2xl bg-navy-50 border border-navy-100 flex items-center justify-center text-navy-900 text-3xl font-extrabold shadow-sm dark:bg-[#00215E]/10 dark:border-[#00215E]/20">
                  {teacherName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <h3 className="font-extrabold text-lg text-navy-950">{teacherName}</h3>
              ...
              <div className="border-t border-gray-100 pt-4 space-y-3.5">
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Registration/ID Number</span>
                  <span className="text-sm font-semibold text-gray-800">{profile?.registrationNumber || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Official Email</span>
                  <span className="text-sm font-semibold text-gray-800">{profile?.email || 'N/A'}</span>
                </div>
                {profile?.phone && (
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Contact Number</span>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-6 md:col-span-2 space-y-6">
            <div>
              <h2 className="text-lg font-bold text-navy-950 flex items-center space-x-2">
                <Key className="h-5 w-5 text-[#00215E] dark:text-sky-400" />
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="pt-2 flex justify-end">
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-[#00215E] hover:opacity-90 text-white font-bold py-3 px-6 rounded-xl transition-all duration-150 text-sm shadow-sm flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <>
