...
- **Comprehensive Popup & Modal Dialog Dark Mode Overhaul**:
  - **High-Contrast Modal Footers & Dialog Cards**: Pinned dialog cards and footers automatically map light utility classes (`bg-white` -> `#1e293b`/`#0f172a`, `bg-gray-50/80`, `bg-slate-50/80`, `bg-gray-50`, `bg-slate-50` -> deep `#0f172a` / `#1e293b`), resolving washed-out light gray footer strips.
  - **Visible Secondary & Cancel Buttons**: Outlined secondary buttons (`border-gray-200`, `text-gray-600`, `text-gray-500`, etc.) are rendered with distinct `#1e293b` backgrounds, crisp `#e2e8f0` (slate-200) text, and `#475569` (slate-600) borders with hover transitions (`#334155` / `#ffffff`), permanently eliminating invisible white-on-white text issues.
  - **Segmented Toggle Pill Tracks**: Offline provisioning toggle switches (`AdminFormModal`, `TeacherFormModal`) feature dark recessed tracks (`bg-[#0b1120]`, border `#334155`), with active pill items highlighted in slate-800 with white text and sky-400 icons, while inactive options remain legible with muted `#94a3b8` text and icons.
  - **Universal Popup Fallback Rule (`frontend/src/index.css`)**: Global cascading dark rules for `.dark .fixed.inset-0` and `.dark [role="dialog"]` ensure all modals, confirm dialogs, slide-over drawers, and child popups automatically inherit high-contrast surfaces, borders, and inputs even if third-party or newly created components omit explicit classes.
